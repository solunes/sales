...
class Sales {
    ...
    public static function generateCompany($array, $external_code) {
        if(!\Solunes\Sales\App\Company::where('external_code', $external_code)->first()){
            $item = new \Solunes\Sales\App\Company;
    ...
    public static function generateContact($array, $external_code) {
        if($item = \Solunes\Sales\App\Contact::where('external_code', $external_code)->first()){
        } else {
            $item = new \Solunes\Sales\App\Contact;
            $item->external_code = $external_code;
        }
        foreach($array as $array_key => $array_item){
            $item->$array_key = $array_item;
    ...
    public static function exportCompany($item) {
        ...
        $properties = \Sales::generateHubspotField($item, $array);
        $fixed_item['properties'] = $properties;
        $item = \Sales::generateHubspotQuery('company', $item, $fixed_item);
        return $item;
    }
    ...
    public static function exportContact($item) {
        ...
        $properties = \Sales::generateHubspotField($item, $array);
        $fixed_item['properties'] = $properties;
        $item = \Sales::generateHubspotQuery('contact', $item, $fixed_item);
        return $item;
    }
    ...
    public static function exportDeal($item) {
        ...
        $properties = \Sales::generateHubspotField($item, $array);
        $fixed_item['properties'] = $properties;
        $item = \Sales::generateHubspotQuery('deal', $item, $fixed_item);
        return $item;
    }
